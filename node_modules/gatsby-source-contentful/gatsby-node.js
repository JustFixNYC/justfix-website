...
exports.sourceNodes = async ({
  ...
  const online = await isOnline(); // If the user knows they are offline, serve them cached result
  ...
  const createSyncToken = () => `${pluginConfig.get(`spaceId`)}-${pluginConfig.get(`environment`)}-${pluginConfig.get(`host`)}`; // Get sync token if it exists.


  let syncToken;

  if (!pluginConfig.get(`forceFullSync`) && store.getState().status.plugins && store.getState().status.plugins[`gatsby-source-contentful`] && store.getState().status.plugins[`gatsby-source-contentful`][createSyncToken()]) {
